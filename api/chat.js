  ...
  parameters: {
    ...
    properties: {
      from: {
        type: "string",
        description:
          'Origin/starting point of route planning (e.g., Postal code or landmark like "Jurong East")'
      },
      to: {
        type: "string",
        description:
          'Destination/ending point of route planning (e.g., Postal code or landmark like "Woodlands MRT")'
      },
      reply_message: {
        type: "string",
        description:
          "A short message while action is loading telling the user what you are doing."
      }
    },
...
const selectHabitRouteFunctionDeclaration = {
  ...
  description:
    "Retrieve and view a specific saved habit route that the user selected via typing route name or index provided in chat.",
  parameters: {
    type: "object",
    properties: {
      route_index: {
        type: "number",
        description:
          "The displayed route index number selected by the user from listed habit routes, if they replied with a number like 1, 2, or 3."
      },
      route_name: {
        type: "string",
        description:
          "The route name selected by the user from listed habit routes, if they replied with the route name, for example like Home to Work"
      },
      reply_message: {
        type: "string",
        description:
...
const selectJamFunctionDeclaration = {
  name: "select_jam",
  description:
    "Select and view a specific jammed segment mappin along a route that user selected via typing index. Users can also ask to go to 'next' or 'previous' jam. You can call this function again if user types 'next' or 'previous' to iterate through the jams.",
  parameters: {
    type: "object",
    properties: {
      jam_index: {
        type: "string",
        description:
          "The displayed jam index number selected by the user from jams in selected route. If the user says a specific number, output that number (e.g., '3'). If they ask for the next jam, output exactly 'next'. If they ask for the previous jam, output exactly 'previous'."
      },
      reply_message: {
        type: "string",
        description:
...
const rerouteFunctionDeclaration = {
  ...
  description:
    "After selecting a map pin, users can choose to recalculate route from an earlier road segment to avoid the jam by typing 'reroute', 'I want to avoid this jam' etc.",
  parameters: {
    type: "object",
    properties: {
      reply_message: {
...
const rerouteDecisionFunctionDeclaration = {
  ...
  description:
    "ONLY trigger this if an alternate route has already been generated. After computing a new alternate route, users can choose to accept or reject the new generated alternate route. For example, 'accept', 'reject', 'yes', 'no'.",
  parameters: {
    type: "object",
    properties: {
      reroute_decision: {
        type: "boolean",
        description:
          "If user plans to reroute, for e.g by saying 'Accept' or 'Yes', set to true. If user rejects e.g 'Reject', 'Decline' or 'No', set to false."
      },
      reply_message: {
        type: "string",
        description:
...
const submitJourneyFeedbackFunctionDeclaration = {
  ...
  description:
    "Submits feedback from the FAST Sentinel journey panel while the journey is active.",
  parameters: {
    type: "object",
    properties: {
      feedback_type: {
      ...
      reply_message: {
        type: "string",
        description:
          "A short message telling the user feedback is being submitted. Thank user for contribution."
      }
    },
  ...
  try {
    ...
    if (response.functionCalls && response.functionCalls.length > 0) {
      ...
      return res.status(200).json({
        ...
        text:
          functionCall.args.reply_message ||
          `Triggering ${functionCall.name}...`
      });
    }
    ...
    console.error("FASTbot Vercel Error:", e);
